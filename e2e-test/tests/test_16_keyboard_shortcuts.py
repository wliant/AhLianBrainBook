...
class TestKeyboardShortcuts:
    def test_question_mark_opens_shortcuts_dialog(self, home: Page):
        home.keyboard.press("?")
        expect(home.get_by_role("heading", name="Keyboard Shortcuts")).to_be_visible(timeout=5000)

    def test_escape_closes_dialog(self, home: Page):
        home.keyboard.press("?")
        expect(home.get_by_role("heading", name="Keyboard Shortcuts")).to_be_visible(timeout=5000)
        home.keyboard.press("Escape")
        expect(home.get_by_role("heading", name="Keyboard Shortcuts")).not_to_be_visible(timeout=3000)

    def test_ctrl_k_opens_search(self, home: Page):
        home.keyboard.press("Control+k")
        expect(home).to_have_url(re.compile(r".*/search.*"), timeout=5000)
